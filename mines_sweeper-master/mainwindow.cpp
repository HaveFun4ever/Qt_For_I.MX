...
double factor_Scale=1;//缩放因子(自定义),其实最后限制30*19了，这个缩放因子在14寸以上屏幕不会用到。
bool IsScale=false;//是否缩放(缩放了的话鼠标坐标也要缩放)
bool blockIsScale = false;
    ...
    ui(new Ui::MainWindow)
{
    ...
    QVBoxLayout *viewLayout = new QVBoxLayout(ui->mainWidget);
    viewLayout->setContentsMargins(0, 0, 0, 0);  // 去除边距，让 _view 贴边
    viewLayout->setSpacing(0);
    _view = new QGraphicsView(ui->mainWidget);
    viewLayout->addWidget(_view);
    qDebug()<<_view->pos()<<endl;

    connect(ui->flagButton,&QPushButton::clicked,this,&MainWindow::on_flag);
    connect(ui->boomButton,&QPushButton::clicked,this,&MainWindow::off_flag);
    _scene=new mineSweepScene;

    _view->setScene(_scene);
    qDebug()<<_scene->width()<<endl<<this->width()<<endl;
    if(_scene->width()>this->width()){
        _view->scale(this->width()/_scene->width()/factor_Scale,this->width()/_scene->width()/factor_Scale);
        IsScale=false;
        blockIsScale = true;
    }
...
void MainWindow::on_actionGG_triggered()
{
    close();
}
...
void MainWindow::mousePressEvent(QMouseEvent *event){
    ...
    QPoint viewPos = _view->mapFrom(this, event->pos());
    QPointF scenePos = _view->mapToScene(viewPos);

    double px = scenePos.x();
    double py = scenePos.y();
    QGraphicsItem *item = _view->itemAt(viewPos);

    _cItem=dynamic_cast<cellItem*>(item);//或 _cItem=(cellItem*)item;

    if(item!=nullptr){//点击在地图范围之外不予响应
        qDebug()<<item->pos().x()<<endl<<item->pos().y()<<endl;
        double cw = IsScale ?  item->boundingRect().width()*this->width()/_scene->width()*factor_Scale : item->boundingRect().width();
        double ch = IsScale ?  item->boundingRect().height()*this->width()/_scene->width()*factor_Scale : item->boundingRect().height();
        qDebug()<<cw<<endl<<ch<<endl;
//        switch(event->button())
        switch(ifFlag)
        {
        case false:
            ...
            if(INITIAL==_cItem->getStatus() || QUESTION==_cItem->getStatus()){
                  qDebug()<<Field->_cells[item->pos().x()/cw][item->pos().y()/ch]<<endl;
//                qDebug()<<"item->pos().x()/cw:"<<item->pos().x()/cw<<endl<<item->pos().x()/cw<<endl;
                ...
                case -1:
                    ...
                    QMessageBox::information(this,"Result","Boom Boom Boom New game will start...");
...
void MainWindow::on_flag()
{
    ifFlag=true;
    ui->boomButton->setEnabled(true);
    ui->flagButton->setEnabled(false);

}

void MainWindow::off_flag()
{
    ifFlag=false;
    ui->boomButton->setEnabled(false);
...
void MainWindow::block_blank(double px,double py,int x,int y){
    ...
    QGraphicsItem *item = _scene->itemAt(px, py, QTransform());

    double cw = blockIsScale ?  item->boundingRect().width()*this->width()/_scene->width()*factor_Scale : item->boundingRect().width();
    double ch = blockIsScale ?  item->boundingRect().height()*this->width()/_scene->width()*factor_Scale : item->boundingRect().height();
    ...
    queue<pair<Point_Px,int>> que;//保存鼠标坐标和数矩阵坐标
    ...
    while(!que.empty()){
        ...
        for(int i=0;i<8;++i){//8个方向扩展
            ...
            QGraphicsItem *item = _scene->itemAt(npx, npy, QTransform());

            if (item == nullptr) {
                qDebug() << "itemAt returned nullptr at:" << npx << npy;
                continue;  // 跳过，不处理
            }
            _cItem = dynamic_cast<cellItem*>(item);
            if (_cItem == nullptr) {
                continue;  // 不是 cellItem，跳过
            }

//            qDebug()<<npx<<"   "<<npy<<endl;
//            qDebug()<<"nx ny :::"<<nx<<"  "<<ny<<endl;
              qDebug()<<item->pos().x()<<endl<<item->pos().y()<<endl;
            ...
            if(0==Field->_cells[nx][ny]){
                ...
                qDebug()<<"nx: "<<nx<<"ny: "<<ny<<endl;
                qDebug()<<Field->_cells[nx][ny]<<endl;
