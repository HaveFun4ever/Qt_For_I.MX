...
#define BUFFER_SIZE  16384       // 解码缓冲区大小
...
// 播放 MP3 文件，支持从指定秒数开始
int play_mp3(const char *filename, int start_sec)
{
    ...
    // ===== 跳转到指定时间 =====
    if (start_sec > 0) {
        off_t target_sample = (off_t)start_sec * sample_rate;
        if (total_samples > 0 && target_sample >= total_samples) {
            fprintf(stderr, "错误: 起始时间 (%d 秒) 超过音频总时长\n", start_sec);
            goto cleanup;
        }
        off_t seeked = mpg123_seek(mh, target_sample, SEEK_SET);
        if (seeked == MPG123_ERR) {
            fprintf(stderr, "跳转失败: %s\n", mpg123_strerror(mh));
            goto cleanup;
        }
        printf("已跳转到: %.1f 秒 (样本 %ld)\n", (double)seeked / sample_rate, (long)seeked);
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        printf("用法:\n");
        printf("  播放:       %s <MP3文件路径> [起始秒数]\n", argv[0]);
        printf("  从120秒开始 %s song.mp3 120\n", argv[0]);
        printf("  暂停:       %s pause\n", argv[0]);
        printf("  继续:       %s resume\n", argv[0]);
        printf("  停止:       %s stop\n", argv[0]);
        return 1;
    ...
    // 播放文件
    int ret = play_mp3(argv[1], start_sec);
